...
export default function PerformanceJourney() {
  ...
  const [activeLineStage, setActiveLineStage] = useState<JourneyStage>('PREPARE');
  ...
  useEffect(() => {
    ...
    // Start moving the line progress to Hydrate (Dot 2) on mount
    const timer = setTimeout(() => {
      setActiveLineStage('HYDRATE');
    }, 100);
  ...
  // Auto-demo mode sequence cycling with dynamic duration
  useEffect(() => {
    if (!isAutoPlayActive) return;

    // Wait 2 seconds on REPEAT, 8 seconds on other stages
    const duration = activeStage === 'REPEAT' ? 2000 : 8000;

    const timer = setTimeout(() => {
      setActiveStage((currentStage) => {
        const currentIdx = stagesList.indexOf(currentStage);
        const nextIdx = (currentIdx + 1) % stagesList.length;
        const nextStage = stagesList[nextIdx];

        // The line target (activeLineStage) should be the stage AFTER nextStage
        const lineIdx = (nextIdx + 1) % stagesList.length;
        const lineStage = stagesList[lineIdx];

        setDirection('forward');
        setActiveLineStage(lineStage);
        return nextStage;
      });
    }, duration);

    return () => clearTimeout(timer);
  }, [isAutoPlayActive, activeStage]);

  // Pause auto-demo immediately on user click
  const handleStageSelection = (stage: JourneyStage) => {
    setIsAutoPlayActive(false);

    const prevIdx = stagesList.indexOf(activeStage);
    const nextIdx = stagesList.indexOf(stage);
    setDirection(nextIdx >= prevIdx ? 'forward' : 'backward');

    setActiveLineStage(stage);
  ...
  return (
    <section 
      className="relative bg-white pt-16 md:pt-24 pb-6 md:pb-8 px-6 md:px-12 lg:px-24 overflow-hidden border-t border-neutral-100 font-sans" 
        ...
        <div className="relative mb-10 max-w-3xl mx-auto px-4">
          ...
          <div className="absolute left-6 right-6 top-5 -translate-y-1/2 h-[3px] bg-neutral-100 z-0 rounded-full overflow-hidden">
            <div 
              ...
              style={{ 
                width: `${(stagesList.indexOf(activeLineStage) / (stagesList.length - 1)) * 100}%`,
                backgroundColor: getStageColor(activeLineStage),
                transitionProperty: 'width, background-color',
                transitionDuration: activeLineStage === 'PREPARE' && activeStage === 'REPEAT'
                  ? '0ms'
                  : isAutoPlayActive
                    ? '8000ms'
                    : '600ms',
                transitionTimingFunction: isAutoPlayActive ? 'linear' : 'ease-out'
              }}
            />
          </div>
